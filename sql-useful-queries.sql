...
-- Get XML Data Directly in Output
WITH XML_CHUNKS AS (
    SELECT 
        SUBSTR(e.xml_data, 1, 32767) AS chunk1,
        SUBSTR(e.xml_data, 32768, 32767) AS chunk2,
        SUBSTR(e.xml_data, 65535, 32767) AS chunk3,
        SUBSTR(e.xml_data, 98302, 32767) AS chunk4,
        SUBSTR(e.xml_data, 131069, 32767) AS chunk5,
        SUBSTR(e.xml_data, 163836, 32767) AS chunk6,
        SUBSTR(e.xml_data, 196603, 32767) AS chunk7,
        SUBSTR(e.xml_data, 229370, 32767) AS chunk8,
        SUBSTR(e.xml_data, 262137, 32767) AS chunk9,
        SUBSTR(e.xml_data, 294904, 32767) AS chunk10
    FROM headers h
    INNER JOIN entries e
    ON h.entry_id = e.entry_id
    WHERE h.header_id = 12345 -- Replace with your actual condition
)
SELECT 
    XMLPARSE(DOCUMENT 
        CAST(
        ...
        AS VARCHAR(327670) FOR BIT DATA) PRESERVE WHITESPACE
    ) AS full_xml_data
FROM XML_CHUNKS;