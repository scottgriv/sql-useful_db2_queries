...
-- Replace Email
SELECT DISTINCT USERID   AS "DIR_User",
                DOMROUTE AS "DIR_Email",
                LEFT(DOMROUTE, LOCATE_IN_STRING(DOMROUTE, '@') - 1) CONCAT
                '@to_email_domain.com'
FROM   QUSRSYS.QATMSMTPA
WHERE  LOCATE_IN_STRING(DOMROUTE, '@') > 1
       AND UCASE(DOMROUTE) LIKE '%from_email_domain.com%'; 
